...
for j in l2:
   ...
   for i in range(10):
      ...
      print('sizes: ',len(X_test),len(y_test))
...
if l2plot:
   ...
   fig,ax=plt.subplots()
   ax.errorbar(l2,lists['ylist'], lists['error1'], color = 'red',marker = 'o')
   ax.set_xlabel('% of database')
   ax.set_ylabel('R2-CVM',color='red',fontsize=14)
   # ax.set_ylim(0.5,0.8)
   ax2 = ax.twinx()
   ax2.errorbar(l2,lists['wlist'], lists['error2'], color = 'blue', marker = 'o')
   ...
   plt.show()
   fig.savefig('Results/figures/Mg_size_db2.png',format = 'png',dpi = 100,bbox_inches='tight')
   plt.close('all')
exit()