...
if (narg>=1) then
   do ia=1, narg
      call getarg(ia, atmp)
      if (trim(atmp)=='-i') call getarg(ia+1, fcif)
...
call get_rdf(elneg, b, anames, rf, bfac, rmin, rmax, ngrid, grid_elneg, vol)
call get_rdf(pol, b, anames, rf, bfac, rmin, rmax, ngrid, grid_pol, vol)
call get_rdf(rvdw, b, anames, rf, bfac, rmin, rmax, ngrid, grid_rvdw, vol)
